...
m_lwage_educ=lm(lwage~educ,data=wage)
m_lwage_educ_aug=augment(m_lwage_educ)
```
...
ggplot(data = m_lwage_iq_aug, aes(x = .fitted, y = .resid)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(x = "Fitted values", y = "Residuals")
ggplot(data = m_lwage_educ_aug, aes(x = .fitted, y = .resid)) +
  geom_point(alpha = 0.6) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(x = "Fitted values", y = "Residuals")
ggplot(m_lwage_iq_aug) +
  ...
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  labs(x = "Theoretical quantiles", y = "Standardized residuals")
ggplot(m_lwage_educ_aug) +
  geom_qq(aes(sample = .std.resid)) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  labs(x = "Theoretical quantiles", y = "Standardized residuals")
ggplot(data = wage, aes(x = educ, y = lwage)) +
  geom_point() +
  stat_smooth(method = "lm", se = FALSE)
```
...
# probability of a signle case not being an outlier is therefore the complement 
...
outliersx <- Bayes.outlier(m_lwage_iq, k = 4.034)
outliersx_df <- data.frame(probability = outliersx$prob.outlier,
                          case = 1:length(outliersx$prob.outlier))
ggplot(outliersx_df, aes(ymax = probability, x = case)) +
  geom_linerange(ymin = 0) +
  labs(y = "Probability")
(prob_outlier <- pnorm(-4.034) + pnorm(4.034, lower.tail = FALSE))
...
wage_red=as.data.frame(wage_red)
