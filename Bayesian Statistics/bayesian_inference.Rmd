...
```{r E2-t-dist}
nc_prm = filter(nc, premie == 'premie')
summary(nc_prm)
ggplot(data = nc_prm, aes(x =weight)) +
  geom_histogram(binwidth = 5)
bayes_inference(y = weight, data = nc_prm, 
                statistic = "mean", type = "ci",  
                prior_family = "JZS", mu_0 = 7.7, rscale = 1,
                method = "simulation",
                cred_level = 0.95)
bayes_inference(y = weight, data = nc_prm, 
                ...
                method = "theoretical",show_plot=FALSE,
                cred_level = 0.95)
weight_prm = bayes_inference(y = weight, data = nc_prm, 
                              statistic = "mean", type = "ci",  
                              prior_family = "JZS", mu_0 = 7.7, rscale = 1,
                              method = "simulation",
                              cred_level = 0.95)
prsamples = as.data.frame(weight_prm$samples)
prnsim = nrow(prsamples)
prsamples = mutate(prsamples, y_pred = rnorm(nsim, mu, sqrt(sig2)))
ggplot(data = prsamples, aes(x = y_pred)) + 
  geom_histogram(aes(y = ..density..), bins = 100) +
  geom_density() + 
  xlab(expression(y[new]))
dplyr::select(prsamples, mu, y_pred) %>%
  map(quantile, probs=c(0.025, 0.50, 0.975))
```
...
pre_diff_post=PSS_before-PSS_after
```
